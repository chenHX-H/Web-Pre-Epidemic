...
export default {
    setup() {
        ...
        function switchMap(name) {
            ...
            if(word=="县" || word=="区" ){
                /* 有特殊作用，因为接口地址，以最后是否 有 '_full' 来判断返回的数据；
                有该参数则说明下面还有子集，若没有表明地理位置只勘测到当前级别，返回当前级别的大图，
                因为数据是自己写的，所以无法判断是否是末尾节点，用此方法来屏蔽因为参数错误导致的 get 404错控制台报错 */
                /* 若得到完整正确的 区域码文件，则可优化/ 取消此函数 */
